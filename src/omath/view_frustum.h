...
/* The camera's view frustum, radar approach in world space. Double precision.
...
public:
	 ...
	 * i. e. every frame. Takes unnormalized vectors just like the lookAt() matrix. */
	void set_camera_vectors( const omath::dvec3& pos, const omath::dvec3& front, const omath::dvec3& up );

	virtual ~view_frustum();

	t_intersect is_point_in_frustum( const omath::dvec3& point ) const;

	t_intersect is_sphere_in_frustum( const omath::dvec3& center, const double& radius ) const;

	t_intersect is_box_in_frustum( const daabb &box ) const;
