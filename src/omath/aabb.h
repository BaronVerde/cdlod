...
struct aabb_t {
	...
	aabb_t( const aabb_t<T> &rhs ) = default;
	aabb_t( aabb_t<T> &&rhs ) = default;
	aabb_t &operator=( const aabb_t<T> &rhs ) = default;
	aabb_t &operator=( aabb_t<T> &&rhs ) = default;
	...
	bool intersect_sphere_sq( const omath::vec3_t<T>& center, T radius_sq ) const {
		return min_distance_from_point_sq( center ) <= radius_sq;
	}

	bool is_inside_sphere_sq( const omath::vec3_t<T>& center, T radius_sq ) const {
...
typedef aabb_t<unsigned int> uaabb;

}

template<typename T>
std::ostream& operator<<( std::ostream& o, const omath::aabb_t<T>& b ) {
	o << '(' << b.m_min << '/' << b.m_max << ')';
	return o;
}
