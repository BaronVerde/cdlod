...
void ui_overlay::setup() {
	...
	m_AttribLocationTex = glGetUniformLocation( m_shader->get_program(), "Texture");
	m_AttribLocationPosition = glGetAttribLocation( m_shader->get_program(), "Position");
	m_AttribLocationUV = glGetAttribLocation( m_shader->get_program(), "UV");
	m_AttribLocationColor = glGetAttribLocation( m_shader->get_program(), "Color");
    ...
    m_shader->use();
    set_uniform( m_shader->get_program(), "Texture", FONT_TEXTURE_UNIT );
    m_shader->un_use();
    io.Fonts->TexID = (void *)(intptr_t)m_fontTexture;
    if( !io.Fonts->IsBuilt() )
    	logbook::log_msg( logbook::RENDERER, logbook::ERROR,"The overlay font atlas was not built correctly." );
...
void ui_overlay::render(const double delta_time) {
	if( !m_showUI ) {
	...
	ImGui::Begin( "Keys:" );
	ImGui::Text( "Ctrl-U to toggle UI display" );
	ImGui::Text( "W/A/S/D/Y/X/shift to move" );
	ImGui::Text( "P to print camera position" );
	ImGui::Text( "F to toggle wireframe mode" );
	...
	ImGui::SameLine(); ImGui::Text( "   %.1lf fps", 1 / delta_time );
	ImGui::SameLine();
