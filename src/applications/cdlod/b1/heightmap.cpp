...
heightmap::heightmap( const std::string &filename, const bit_depth depth ) :
	...
	if( B16 == depth )
		...
		values_16 = stbi_load_16( m_filename.c_str(), &w, &h, &num_channels, 1 );
	if( B8 == depth ) {
		// load the data, single channel 8
		values_8 = stbi_load( m_filename.c_str(), &w, &h, &num_channels, 1 );
	}
	...
	s << "Heightmap '" << m_filename << "', texture unit " << HEIGHTMAP_TEXTURE_UNIT <<
			", " << m_extent.x << '*' << m_extent.y << ", " << num_channels <<
			" channel(s) loaded. Size in memory : " << size_in_kb << "kB.";
