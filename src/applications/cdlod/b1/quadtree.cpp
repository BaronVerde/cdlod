...
bool quadtree::create() {
	...
	for( unsigned int i=0; i < m_nodeCount; ++i ) {
		s.str( std::string() );
		node *n{ &m_allNodes[i] };
		s << "Node " << i << " Level " << n->get_level() << " BB " << n->get_world_aabb();
		if( n->is_leaf() )
			s << "; is leaf node.";
		else {
			s << "; child lvls: " << n->get_tl()->get_level() << '/' <<
					n->get_tr()->get_level() << '/' << n->get_bl()->get_level() <<
					'/' << n->get_br()->get_level();
		}
		// TODO orf_n::logbook::log_msg( orf_n::logbook::TERRAIN, orf_n::logbook::INFO, s.str() );
	}
	return true;
}
